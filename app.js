// Device UI with: landing → login/signup/reset → home/friends/groups/room
...
function authNav(which){
  // pages: landing, login, signup, reset
  ['auth-landing','auth-login','auth-signup','auth-reset'].forEach(p=> show(document.getElementById('page-'+p), p===which));
}
function setAuthUI(needAuth){
  if (needAuth){
    authNav('auth-landing'.replace('auth-','')); // -> landing
  }
  show(pageEl('home'), false);
  show(pageEl('friends'), false);
  show(pageEl('groups'), false);
  show(pageEl('room'), false);
...
function wirePresence(){
  ...
  infoRef.on('value', async (snap)=>{
    ...
    // if your backend mirrors to Firestore, the user doc will get "online:true" automatically
  });
}
...
function nameWithDot(name, online){
  return `${escapeHtml(name||'User')}${online ? uDot(true) : ''}`;
}
